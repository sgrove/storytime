...
| FR-010..FR-014 Editor real-time protocol | Partial | Full `story:{id}` join + required mutation/generation/deploy events and persisted writes are live, with contract tests tracking required FR-011/FR-012 event coverage. Additional dialogue-centric flows (`generate_dialogue`, `generate_all_dialogue`) and queue-aware editor controls are now implemented, including enriched per-job runtime diagnostics (speaker/page/text context, queue position, retries, attempt metadata); payload-shape parity rehearsal is still in progress. |
...
| FR-031..FR-039 Reader experience | Partial | Reader implements loading/error states, scene rendering, navigation/swipe, narration/dialogue playback, page dialogue sequence controls, active speaker highlighting, word highlighting, narrate/read-alone mode, and music crossfade/volume; final UX polish remains. |
