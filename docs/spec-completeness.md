...
| FR-010..FR-014 Editor real-time protocol | Partial | Full `story:{id}` join + required mutation/generation/deploy events and persisted writes are live, with contract tests tracking required FR-011/FR-012 event coverage. Additional dialogue-centric flows (`generate_dialogue`, `generate_all_dialogue`) and queue-aware editor controls are now implemented; payload-shape parity rehearsal is still in progress. |
